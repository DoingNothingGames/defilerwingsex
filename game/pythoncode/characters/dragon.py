...
class Dragon(Fighter):
    ...
    @property
    def can_grow(self):
        return self.mana > 0 and self.lust > 0
    ...
    def contains_special_place_label(self, lb, data):
        for stage in self.special_places.values():
            if not stage in data.special_places:
                continue
            if data.special_places[stage][1] == lb:
                return True
        return False
    
    def contains_special_place(self, name):
        if name in self.special_places.keys():
            return True
        return False
