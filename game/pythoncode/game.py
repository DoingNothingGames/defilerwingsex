...
class Game(store.object):
    ...
    def next_year(self):
        ...
        self.solve_thief_actions()
        
        self.solve_knight_actions()		
    ...
    def removeVisited(self, choices, visitStack):
        result = []
        for (lb, chance) in choices:
            if lb in visitStack:
                continue
            if self._dragon.contains_special_place_label(lb, data):
                continue
            result.append((lb, chance))
            
        return result
        
    def appendVisited(self, visitStack, lb):
        if lb in visitStack:
            return
        visitStack.insert(0, lb)
        if len(visitStack) > 5:
            visitStack.pop()
